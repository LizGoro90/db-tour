...
func main() {
	...
	// across all supported databases in mind. Sometimes that won't be enough for
	// all your needs. If that's your case, you can also use the SQL builder or
	// even raw SQL depending on how complex your query is.
	//
	// The SQLBuilder is defined in the upper.io/db.v3/lib/sqlbuilder package,
	// you can see all its available methods on
	// https://godoc.org/upper.io/db.v3/lib/sqlbuilder#SQLBuilder.
	q := sess.SelectFrom("books")

	// q is a sqlbuilder.Selector, you can chain any of its other methods that
	// return the same type. See
	// https://godoc.org/upper.io/db.v3/lib/sqlbuilder#Selector
	q = q.OrderBy("title")

	// Remember that queries are immutable, here p is a completely independent
	// query.
	...
	// The Iterator method is a way to iterator over large sets of results one by
	// one.
	...
	// An iterator must be closed to free up related resources.
